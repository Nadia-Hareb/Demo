    ...
    <v-card>
      <form class="elevation-1 pa-6">
        ...
        <v-card-actions>
          ...
          <v-btn @click="submit" color="red darken-1" text> Save </v-btn>
        </v-card-actions>
      </form>
    </v-card>
  </v-dialog>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      if (!this.$v.$invalid) {
        if (this.id) {
          this.$store.dispatch("updateRessourceTitle", {
            id: this.id,
            name: this.name,
            number: this.number,
  ...
  mounted() {
    if (this.ressource) {
      ...
      this.effectiveDate = this.ressource.effectiveDate.substring(
        0,
        this.ressource.effectiveDate.length - 9
      );
